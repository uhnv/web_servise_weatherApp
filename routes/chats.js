 ...
 * @apiSuccess {String} members.email The email for the member in the chat
 * 
...
/**
 * @api {get} /chats Request to get all chatrooms associated with memberid
 * @apiName GetChats
 * @apiGroup Chats
 *
 ...
 * @apiSuccess {String} chat.name The name for the chatroom
 * @apiSuccess {String} chat.chatid The id for the chatroom
 *
...
router.get("/", (request, response, next) => {
    ...
    let query = `SELECT Chats.Name, Chats.ChatID 
                FROM Chats INNER JOIN ChatMembers 
                ON Chats.ChatID = ChatMembers.ChatID 
                WHERE MemberID = $1`
    let values = [request.decoded.memberid]
    pool.query(query, values)
        .then(result => {
